...
impl ProjectRepository {
    ...
    pub fn get_project_by_id(db: &DatabaseManager, id: &str) -> Result<Option<ProjectRecord>, String> {
        db.with_conn(|conn| {
            let mut stmt = conn.prepare(
    ...
    pub fn get_project(db: &DatabaseManager, project_id: &str) -> Result<Option<ProjectRecord>, String> {
    ...
    pub fn delete_projects_batch(db: &DatabaseManager, project_ids: &[String]) -> Result<(), String> {
    ...
    pub fn delete_segment(db: &DatabaseManager, project_id: &str, segment_id: &str) -> Result<(), String> {
        Self::delete_segments_batch(db, project_id, &[segment_id.to_string()])
    }

    pub fn delete_segments_batch(db: &DatabaseManager, project_id: &str, segment_ids: &[String]) -> Result<(), String> {
        ...
        db.with_conn_mut(|conn| {
            ...
            for seg_id in segment_ids {
                ...
                tx.execute("DELETE FROM segments WHERE project_id = ?1 AND id = ?2", params![project_id, seg_id])?;
            ...
            let remaining_ids: Vec<String> = {
                let mut stmt = tx.prepare("SELECT id FROM segments WHERE project_id = ?1 ORDER BY position ASC")?;
                let rows = stmt.query_map(params![project_id], |row| row.get(0))?;
                let res = rows.filter_map(|r| r.ok()).collect();
                res
            };
    ...
    pub fn insert_segment_at(
        ...
        db.with_conn_mut(|conn| {
            ...
            tx.execute(
                ...
                params![seg_id, project_id, position as i64, text, "Đọc tự nhiên", "pending", now],
            )?;
    ...
    pub fn swap_segment_positions(
        ...
        db.with_conn_mut(|conn| {
            ...
            let target_pos = if direction == "up" {
                ...
                if current_pos >= max_pos { return Ok(()); }
                current_pos + 1
            };
            ...
            tx.execute("UPDATE segments SET position = ?1 WHERE id = ?2", params![target_pos as i64, segment_id])?;
            tx.execute("UPDATE segments SET position = ?1 WHERE id = ?2", params![current_pos as i64, other_id])?;
    ...
    pub fn get_segments_for_project(db: &DatabaseManager, project_id: &str) -> Result<Vec<SegmentRecord>, String> {
        db.with_conn(|conn| {
            let mut stmt = conn.prepare(
                "SELECT id, project_id, position, text, prompt, status, attempts, audio_path, duration_ms,
                        error_code, error_message, created_at, updated_at, fingerprint, output_fingerprint,
                        attempt_count, next_retry_at, queued_at, started_at, finished_at, lease_owner,
                        lease_expires_at, last_error_code, last_error_message, cancel_requested, state_revision, output_size, voice
                 FROM segments WHERE project_id = ?1 ORDER BY position ASC",
            )?;

            let seg_iter = stmt.query_map(params![project_id], |row| {
    ...
    pub fn mark_project_segments_queued(db: &DatabaseManager, project_id: &str) -> Result<usize, String> {
    ...
    pub fn claim_next_task(
        ...
        let now_ms = Utc::now().timestamp_millis();
        let lease_expires_at = now_ms + (lease_duration_secs * 1000) as i64;
            ...
            if let Some(seg_id) = candidate_id {
                ...
                if updated > 0 {
                    let seg = {
                        ...
                        stmt.query_row(params![seg_id], |row| {
                            let pos: i64 = row.get(2)?;
                            let dur: i64 = row.get(8)?;
                            let cancel_req: i32 = row.get(24).unwrap_or(0);
                            let rev: i64 = row.get(25).unwrap_or(0);
    ...
    pub fn get_next_retry_delay_ms(db: &DatabaseManager, project_id: &str) -> Result<Option<u64>, String> {
        let now_ms = Utc::now().timestamp_millis();
        db.with_conn(|conn| {
            let next_retry: Option<i64> = conn.query_row(
            ...
            let seg: SegmentRecord = {
                let mut stmt = tx.prepare(
                    "SELECT id, project_id, position, text, prompt, status, attempts, audio_path, duration_ms,
                            ...
                            attempt_count, next_retry_at, queued_at, started_at, finished_at, lease_owner,
                            lease_expires_at, last_error_code, last_error_message, cancel_requested, state_revision, output_size, voice
                     FROM segments WHERE id = ?1 AND project_id = ?2",
                )?;

                stmt.query_row(params![segment_id, project_id], |row| {
                    let pos: i64 = row.get(2)?;
                    let dur: i64 = row.get(8)?;
                    let cancel_req: i32 = row.get(24).unwrap_or(0);
                    let rev: i64 = row.get(25).unwrap_or(0);
    ...
    pub fn update_source_text(db: &DatabaseManager, project_id: &str, source_text: &str) -> Result<(), String> {
        let now = Utc::now().to_rfc3339();
        db.with_conn(|conn| {
            conn.execute(
    ...
    pub fn delete_segments_for_project(db: &DatabaseManager, project_id: &str) -> Result<(), String> {
        db.with_conn(|conn| {
            conn.execute("DELETE FROM segments WHERE project_id = ?1", params![project_id])?;
            Ok(())
        })
    }
}
...
mod tests {
    ...
    fn test_get_project_and_updates() {
        ...
        let updated_p = ProjectRepository::get_project(&db, "p_test").unwrap().unwrap();
        ...
        ProjectRepository::update_segment_text(&db, "p_test", "s_test_1", "Updated Text", "New Prompt", "fp_new").unwrap();
    ...
    fn test_get_project_by_id_and_segment_repo_helpers() {
        ...
        let updated_proj = ProjectRepository::get_project_by_id(&db, "p_direct").unwrap().unwrap();
        ...
        SegmentRepository::update_text(&db, "p_direct", "s_seg_repo", "Modified text", "Prompt new", "fp_mod").unwrap();
